...
#define SCATTERING_FOAM_BRIGHTNESS 4.0
...
{
    return saturate(-jacobian + foamAmount) * _WindSpeedMultiplier;
}
...
float3 EvaluateScatteringColor(float sssMask, float lowFrequencyHeight, float horizontalDisplacement, float3 absorptionTint, float deepFoam)
{
    ...
    return scatteringTint * (1.f - absorptionTint) * lambertCompensation * (1.0 + deepFoam) * _ScatteringIntensity;
}
#endif
